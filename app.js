...
async function renderDiscover(type) {
  ...
  try {
    ...
    // Fix for movie pagination - use 'movies' instead of 'movie' in URL
...
function bindPlayerControls(data) {
  ...
  if (controlsSecondary && !el('#downloadBtn')) {
    const downloadBtn = document.createElement("a");
    downloadBtn.id = "downloadBtn";
    downloadBtn.textContent = "Download";
    downloadBtn.className = "btn download";
    downloadBtn.target = "_blank";
    downloadBtn.style.marginLeft = "10px"; // Add some spacing
    
    // Set initial download link
    if (data.media_type === "movie") {
      downloadBtn.href = `https://dl.vidsrc.vip/movie/${data.id}`;
    } else if (data.media_type === "tv") {
      downloadBtn.href = `https://dl.vidsrc.vip/tv/${data.id}/${state.currentSeason}/${state.currentEpisode}`;
    }
    
    controlsSecondary.appendChild(downloadBtn);
  }
  ...
  if (serverSelect) {
    serverSelect.addEventListener('change', async (e) => {
      ...
      // Download link doesn't need updating for server changes
    });
  }
  ...
  if (seasonSelect) {
    seasonSelect.addEventListener('change', async (e) => {
      ...
      state.currentEpisode = 1; // Reset to episode 1 when season changes
...
function setupEventListeners() {
  ...
  // Add this new event listener for pagination clicks
  document.addEventListener('click', (e) => {
